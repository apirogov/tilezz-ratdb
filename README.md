# tilezz simple matchstick polygons on Z[zeta_3], perimeter <= 39, free

Simple matchstick polygons -- closed self-avoiding polygonal walks with unit-length edges and turn angles in integer multiples of `2*pi/3` -- on the cyclotomic ring `Z[zeta_3]`, with perimeter up to 39, canonicalised under free (full dihedral symmetry reduction) symmetry. 730680086 sequences.

## Turn-angle units

Z[zeta_3] has no native lattice in tilezz, so this dataset is the order-3 sub-ring of Z[zeta_6] -- the turn directions that are multiples of 2 -- enumerated with `--step 2`. Each stored turn is therefore a Z[zeta_6] turn (an integer multiple of `2*pi/6`, always even) and is `2` times the corresponding Z[zeta_3] turn (a multiple of `2*pi/3`).

To read the sequences as Z[zeta_3] turns, divide every stored value by `2`. For example the regular 3-gon is stored as `2 2 2` and is `1 1 1` in Z[zeta_3]. The web explorer shows the halved Z[zeta_3] form; `tools/decode.py` prints the raw stored Z[zeta_6] values.
