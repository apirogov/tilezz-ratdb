# tilezz simple matchstick polygons on Z[zeta_7], perimeter <= 21, free

Simple matchstick polygons -- closed self-avoiding polygonal walks with unit-length edges and turn angles in integer multiples of `2*pi/7` -- on the cyclotomic ring `Z[zeta_7]`, with perimeter up to 21, canonicalised under free (full dihedral symmetry reduction) symmetry. 1116823845 sequences.

## Turn-angle units

Z[zeta_7] has no native lattice in tilezz, so this dataset is the order-7 sub-ring of Z[zeta_14] -- the turn directions that are multiples of 2 -- enumerated with `--step 2`. Each stored turn is therefore a Z[zeta_14] turn (an integer multiple of `2*pi/14`, always even) and is `2` times the corresponding Z[zeta_7] turn (a multiple of `2*pi/7`).

To read the sequences as Z[zeta_7] turns, divide every stored value by `2`. For example the regular 7-gon is stored as `2 2 2 2 2 2 2` and is `1 1 1 1 1 1 1` in Z[zeta_7]. The web explorer shows the halved Z[zeta_7] form; `tools/decode.py` prints the raw stored Z[zeta_14] values.
