# tilezz simple matchstick polygons on Z[zeta_4], perimeter <= 32, free

Simple matchstick polygons -- closed self-avoiding polygonal walks with unit-length edges and turn angles in integer multiples of `2*pi/4` -- on the cyclotomic ring `Z[zeta_4]`, with perimeter up to 32, canonicalised under free (full dihedral symmetry reduction) symmetry. 435646127 sequences.

Upstream cross-reference: this dataset realises OEIS sequence [`A266549`](https://oeis.org/A266549).
...
Produced by [`tilezz`](https://github.com/apirogov/tilezz) v0.1.0, commit `dd4fd140969d82adddc54331d4d652c1f72d45a6`, via three-stage streaming pipeline (`--mode stream` -> `--mode merge` -> `--mode build`).
