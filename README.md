# tilezz simple matchstick polygons on Z[zeta_10], perimeter <= 18, free

Simple matchstick polygons -- closed self-avoiding polygonal walks with unit-length edges and turn angles in integer multiples of `2*pi/10` -- on the cyclotomic ring `Z[zeta_10]`, with perimeter up to 18, canonicalised under free (full dihedral symmetry reduction) symmetry. 2875831850 sequences.

Upstream cross-reference: this dataset realises OEIS sequence [`A316200`](https://oeis.org/A316200).
